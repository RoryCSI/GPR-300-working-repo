...
void a3ssfx_render(a3_DemoState const* demoState, a3_DemoMode2_SSFX const* demoMode, a3f64 const dt)
{
	...
	if (renderMode == ssfx_renderModePhongDL)
	{
		...
		a3shaderUniformSendFloatMat(a3unif_mat4, 0, currentDemoProgram->ubTransformMVP, ssfxMaxCount_pointLight, demoMode->pointLightMVP->mm); //Doesn't seem to do anything
		...
		glCullFace(GL_FRONT);//To "invert" the unit spheres -> trying the same as Buckstein's shadow pass from previous project
	...
	case ssfx_renderModePhongDL:
		...
		a3framebufferBindColorTexture(writeFBO[ssfx_renderPassLights], a3tex_unit02, 0);//pre-pass diffuse
		a3framebufferBindColorTexture(writeFBO[ssfx_renderPassLights], a3tex_unit03, 1);//pre-pass specular
		a3framebufferActivate(currentWriteFBO);

		//send inverse bias projection mat
		a3shaderUniformSendFloatMat(a3unif_mat4, 0, currentDemoProgram->uPB_inv, 1, projectionBiasMatInv.mm);
		break;
	}
